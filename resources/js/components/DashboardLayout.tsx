...
const DashboardLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  // Normalize URL to handle trailing slashes or query params for active state
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <aside className={cn(
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {navItems.map((item) => {
            const active = currentPath === item.path.replace(/\/$/, '') || (item.path !== '/dashboard' && currentPath.startsWith(item.path));
            return (
              <Link
                key={item.path}
                href={item.path}
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="flex items-center gap-4 px-6 py-4 border-b border-border glass-strong">
          ...
          <h2 className="font-display font-semibold text-foreground">
            {navItems.find((n) => currentPath === n.path.replace(/\/$/, ''))?.label || "Dashboard"}
          </h2>
        </header>
