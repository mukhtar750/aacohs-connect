...
import { GripVertical, FileCode, Trash2, ChevronDown, ChevronUp, Upload, Loader2 } from "lucide-react";
...
const SortableBlock = ({ block: b, onUpdate, onUpdateStyle, onRemove }: SortableBlockProps) => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      onUpdate(b.id, response.data.url);
    ...
    <div ref={setNodeRef} style={style} className="group relative mb-3 border border-transparent hover:border-blue-400/30 rounded-lg p-2 transition-all">
      ...
      {b.type === "image" && (
        <div className="relative">
          <img src={b.content} alt="Block" className="rounded-lg w-full" style={{ width: b.styles?.width || "100%", height: b.styles?.height || "auto" }} />
          
          <div className="opacity-0 group-hover:opacity-100 transition-opacity mt-2 space-y-2 bg-gray-50/80 p-2 rounded-lg border border-gray-200">
            <div className="flex gap-2">
              <input
                value={b.content}
                onChange={(e) => onUpdate(b.id, e.target.value)}
                className="flex-1 text-xs text-gray-600 bg-white border border-gray-300 rounded px-2 py-1.5 outline-none focus:ring-1 focus:ring-blue-400"
              ...
              <button
                ...
                disabled={uploading}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white rounded text-xs font-medium transition-colors"
              >
            ...
            <div className="flex gap-3 items-center">
              <div className="flex items-center gap-1.5">
                ...
                <input
                  ...
                  className="w-20 text-xs text-gray-600 bg-white border border-gray-300 rounded px-2 py-1 outline-none"
                  placeholder="e.g. 100%"
                />
              </div>
              <div className="flex items-center gap-1.5">
